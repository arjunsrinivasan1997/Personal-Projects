...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        return scheme_apply(scheme_eval(first,env), rest.map(lambda a:scheme_eval(a,env)), env)
        # END PROBLEM 5
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Incorrent Number of arguments for function')
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        parameter = expressions.first.second
        body = expressions.second
        value = LambdaProcedure(parameter, body, env)
        env.define(symbol, value)
        return symbol
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        a = bindings.first
        check_form(a,2,2)
        formals = Pair(a.first,formals)
        values = Pair(scheme_eval(a.second.first,env),values)
...
class MuProcedure(UserDefinedProcedure):
    ...
    # BEGIN PROBLEM 16
    def make_call_frame(self, args, env):
        ''' Make Dynamic Scoping call frame'''
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN Extra Credit
            result = scheme_apply(scheme_eval(first,env),rest.map(lambda a:scheme_eval(a,env)),env)
            # END Extra Credit
    return result
