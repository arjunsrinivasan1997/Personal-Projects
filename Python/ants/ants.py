...
class Ant(Insect):
    ...
    def can_contain(self, other):
        if self.container and other.container == False and self.ant == None:
            return True
        else:
            return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self,hive):
        ...
        """
        # BEGIN Problem 3B
        def incrementer(a,place):
            """ Increments a by 1, and changes place to next place closest to hive"""
            a += 1
        ...
        while place is not hive:
            if place.bees and self.min_range <= a and a <= self.max_range:
                return random_or_none(place.bees)
            else:
                a, place = incrementer(a, place)   
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        if bee:
            ...
            self.is_digesting = True        # END Problem 6B

    def action(self, colony):
        # BEGIN Problem 6B
        if self.is_digesting and self.turns_taken_digesting < self.time_to_digest:
            self.turns_taken_digesting += 1
        else:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.index != 1:
            ...
            # doubling damage of all ants behind queen
            place = self.place.exit
            while place.exit:
                    ant = place.ant
                    if ant:
                        if ant.container and ant.ant and ant.ant not in self.ants_doubled:
                            ant.ant.damage *= 2
                            self.ants_doubled.append(ant.ant)
                        elif ant and ant not in self.ants_doubled:
                            ant.damage *= 2
                            self.ants_doubled.append(ant)
                    place = place.exit
            # executing scubathrower action
...
def make_slow(action):
    ...
    """
    # BEGIN Problem EC
    def slow_action_method(colony):
        if colony.time % 2 == 0:
            return action(colony)
    return slow_action_method
    # END Problem EC
...
def make_stun(action):
    ...
    """
    # BEGIN Problem EC
    def stun_action_method(colony):
        return None
    return stun_action_method
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    counter = 0
    action = bee.action
    def applying_action_method(colony):
        nonlocal counter
        if counter < duration:
            ...
            action(colony)
    bee.action = applying_action_method
    # BEGIN Problem EC
