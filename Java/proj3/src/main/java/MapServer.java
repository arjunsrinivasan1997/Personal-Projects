...
public class MapServer {
    ...
    public static List<String> getLocationsByPrefix(String prefix) {
        ...
        LinkedList<String> cleanedWords = graph.getNames().getChildern(cleanedPrefix);
        LinkedList<String> unCleanedWords = new LinkedList<>();
        for(String s: cleanedWords){
            unCleanedWords.add(graph.getCleanedNamestoRealNames().get(s));
        }
    ...
    public static List<Map<String, Object>> getLocations(String locationName) {
        ...
        String cleanedName = graph.cleanString(locationName);
        ArrayList<Long> tempArray = graph.namesToIDs.get(cleanedName);
        for(Long l: tempArray){
            GraphNode n = graph.allNodeswithNames.get(l);
            Map<String,Object> tempMap = new HashMap<>();
            tempMap.put("lat",n.getY());
            tempMap.put("lon",n.getX());
            tempMap.put("id",n.getId());
            tempMap.put("name",graph.getCleanedNamestoRealNames().get(cleanedName));
            result.add(tempMap);
        }
